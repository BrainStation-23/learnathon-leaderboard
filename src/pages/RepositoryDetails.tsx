...
const RepositoryDetails = () => {
  ...
  const prepareMetricsData = () => {
    ...
    const { bugs, vulnerabilities, code_smells } = selectedRepo.sonarData.metrics;
  ...
  const prepareSecurityData = () => {
    ...
    selectedRepo.securityIssues.forEach(issue => {
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">{selectedRepo.repoData.name}</h1>
        </div>
        <div className="flex gap-2">
          ...
          {selectedRepo.sonarData && (
            <Button variant="outline" className="gap-2" asChild>
              <a 
                href={`https://sonarcloud.io/project/overview?id=${selectedRepo.sonarData.project_key}`} 
                target="_blank" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card>
          <CardContent className="p-4 flex flex-col items-center justify-center text-center">
            ...
            <p className="font-medium">{formatTimeAgo(selectedRepo.repoData.last_commit_date)}</p>
            <p className="text-xs text-muted-foreground">{formatDate(selectedRepo.repoData.last_commit_date)}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4 flex flex-col items-center justify-center text-center">
            ...
            <p className="font-medium">{selectedRepo.repoData.commits_count?.toLocaleString() || "N/A"}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4 flex flex-col items-center justify-center text-center">
            ...
            <p className="font-medium">{selectedRepo.securityIssues?.length || 0}</p>
          </CardContent>
        </Card>
      </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <p className="font-medium">{selectedRepo.repoData.full_name}</p>
                    </div>
                    <div>
                      <p className="text-sm text-muted-foreground">License</p>
                      <p className="font-medium">{selectedRepo.repoData.license?.name || "Not specified"}</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <a 
                      href={selectedRepo.repoData.html_url} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="text-blue-600 hover:underline flex items-center gap-1"
                    >
                      {selectedRepo.repoData.html_url}
                  ...
                  <div>
                    ...
                    <p className="font-medium">{formatDate(selectedRepo.repoData.last_commit_date)}</p>
                  </div>
                  
                  <div>
                    <p className="text-sm text-muted-foreground">Updated</p>
                    <p className="font-medium">{formatTimeAgo(selectedRepo.repoData.updated_at)}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <Card>
                  ...
                  <div className="h-32 w-32 rounded-full border-8 border-muted flex items-center justify-center mb-4">
                    <div className="text-center">
                      <p className="text-2xl font-bold">
                        {selectedRepo.sonarData.metrics.technical_debt || "0"}
                      </p>
                    </div>
                  </div>
                  <div className="text-center">
                ...
                <CardFooter>
                  <Button variant="outline" className="w-full text-xs" asChild>
                    <a 
                      href={`https://sonarcloud.io/project/issues?id=${selectedRepo.sonarData.project_key}&resolved=false`} 
                      target="_blank" 
                      rel="noopener noreferrer"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="md:col-span-2">
              ...
              <CardContent className="h-[300px] overflow-auto">
                {selectedRepo.securityIssues && selectedRepo.securityIssues.length > 0 ? (
                  <div className="space-y-3">
                    {selectedRepo.securityIssues.map((issue) => {
                      let severityColor = '';
                      const severity = issue.severity.toLowerCase();
              ...
              {selectedRepo.securityIssues && selectedRepo.securityIssues.length > 0 && (
                <CardFooter>
                  <Button variant="outline" className="w-full gap-2" asChild>
                    <a 
                      href={`${selectedRepo.repoData.html_url}/security/dependabot`} 
                      target="_blank" 
                      rel="noopener noreferrer"
                    >
                      <Bug size={14} />
